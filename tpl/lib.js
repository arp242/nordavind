(function() {
  ...
  jQuery.fn.findNext = function(sel, num, _prev) {
    var arewe, ref;
    if (num == null) num = 1;
    if (_prev == null) _prev = false;
    ref = _prev ? this.prev() : this.next();
    while (true) {
      if (ref.length === 0) return false;
      arewe = ref.is(sel);
      if (arewe) num -= 1;
      if (arewe && num === 0) return ref;
  ...
  window.Slider = Slider = (function() {
    ...
    function Slider(opt) {
      ...
      setpos = function(e) {
        ...
        if (my.opt.move) tip = my.opt.move(my.getpos());
        if (tip != null) {
          if (tooltip === null) {
      ...
      stop = function() {
        if (my.opt.stop) my.opt.stop();
        $('#tooltip').remove();
        return tooltip = null;
      };
      start = function() {
        if (my.opt.start) return my.opt.start();
      };
